...
def discover_new_uri(
    ...
    if archivo_config["release"]["generate_files"]:
        archivo_version.generate_files()

    dataid = archivo_version.build_databus_jsonld(group_info)

    try:
        databusclient.deploy(dataid, archivo_config["databus"]["api-key"])
        logger.info(f"Successfully deployed new Archivo ontology: {dataid['@id']}")
        process_log.append(
            ProcessStepLog(
                status=LogLevel.INFO,
                stepname="Deployment to the Databus",
                message=f"Successfully deployed the new version to the Databus: {dataid['@id']}",
            )
        )
    except Exception as e:
        logger.exception(e)
        process_log.append(
            ProcessStepLog(
                status=LogLevel.ERROR,
                stepname="Deployment to the Databus",
                message=f"Could not deploy new version: {str(e)}",
            )
        )
        return None
    else:
...
def add_dev_uri(
    ...
    dev_uri = urldefrag(dev_uri)[0]

    if not perform_robot_check(dev_uri, process_log, logger):
        return None

    crawling_result = determine_best_content_type(dev_uri, user_output=process_log)
    ...
    parsing_result_turtle = parsing.parse_rdf_from_string(
        crawling_result.response.text, ontology_uri, crawling_result.rdf_type, RDF_Type.TURTLE
    )
    ...
    try:
        ...
        logger.error(f"Exception in rdflib parsing of URI {dev_uri}", exc_info=True)
        process_log.append(
            ProcessStepLog(
                status=LogLevel.ERROR,
                stepname="Load Graph in rdflib",
                message=f"RDFlib couldn't parse the file of {dev_uri}. Reason: {traceback.format_exc()}",
            )
        )
        return None

    group_id, artifact_id = string_tools.generate_databus_identifier_from_uri(
        ontology_uri, dev=True
    )
    ...
    if group_id is None or artifact_id is None:
        process_log.append(
            ProcessStepLog(
                status=LogLevel.ERROR,
                stepname="URI parsing check",
                message=f"ERROR: Malformed ontology ID in document: {ontology_uri}",
            )
        )
        return None
    ...
    archivo_version = ArchivoVersion(
        confirmed_ontology_id=ontology_uri,
        crawling_result=crawling_result,
        parsing_result=parsing_result_turtle,
        databus_version_identifier=databus_version_id,
        access_date=access_date,
