...
def localDiffAndRelease(
    ...
    try:
        ...
        if isEqual:
            ...
            with open(os.path.join(newVersionPath, artifactName + "_type=diff_triples=old.nt")) as old_diff_file:
                print("\n".join(oldTriples), file=old_diff_file)
            with open(os.path.join(newVersionPath, artifactName + "_type=diff_triples=new.nt")) as new_diff_file:
