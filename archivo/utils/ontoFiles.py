...
def getLatestVersionFromArtifactDir(artifactDir):
    try:
        versionDirs = [
            dir
            for dir in os.listdir(artifactDir)
            if os.path.isdir(os.path.join(artifactDir, dir)) and dir != "target"
        ]
...
def altWriteVocabInformation(
    ...
    vocabinfo["ontology-info"] = {
        "non-information-uri": definedByUri,
        "snapshot-url": snapshot_url,
        "semantic-version": semVersion,
        "triples": tripleSize,
        "stars": measureStars(
            rapperErrors,
            licenseViolationsBool,
    ...
    vocabinfo["test-results"] = {
        "consistent": consistentWithImports,
        "consistent-without-imports": consistentWithoutImports,
        "License-I": licenseViolationsBool,
        "License-II": licenseWarningsBool,
        "lode-conform": lodeConform,
    }
    vocabinfo["http-data"] = {
        "accessed": str(accessed),
        "lastModified": lastModified,
        "best-header": bestHeader,
        "content-length": contentLenght,
        "e-tag": etag,
    }
    vocabinfo["logs"] = {
        "rapper-errors": rapperErrors,
        "rapper-warnings": rapperWarnings,
        "nir-header": nirHeader,
        "resource-header": headerString,
    }
...
def getParsedRdf(sourcefile, sourceUri=None, logger=None, silent=True):
    if sourceUri == None:
        process = subprocess.Popen(
            ["rapper", "-g", sourcefile, "-o", "rdfxml"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
    else:
        process = subprocess.Popen(
            ["rapper", "-I", sourceUri, "-g", sourcefile, "-o", "rdfxml"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
...
def parseRDFSource(
    ...
    with open(filepath, "w+") as ontfile:
        process = subprocess.Popen(
            rapperCommand, stdout=ontfile, stderr=subprocess.PIPE
        )
...
def getParsedTriples(filepath, inputFormat=None):
    if inputFormat == None:
        process = subprocess.Popen(
            ["rapper", "-c", "-g", filepath], stderr=subprocess.PIPE
        )
    else:
        process = subprocess.Popen(
            ["rapper", "-c", "-i", inputFormat, filepath], stderr=subprocess.PIPE
        )
...
def inspectMetadata(rootdir):
    ...
    for groupdir in [
        ...
        for artifactDir in [
            ...
            if versionDirs == []:
                print(
                    "Couldnt find version for", groupdir, artifactDir, file=sys.stderr
                )
                continue
