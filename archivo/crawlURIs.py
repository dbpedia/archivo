...
file_ending_mapping = {"application/rdf+xml":"owl", "application/ntriples":"nt", "text/turtle":"ttl", "*/*":'file'}
...
class ArchivoVersion():
  ...
  def generateFiles(self):
    ...
    ontoFiles.parseRDFSource(self.original_file, raw_file_path+"_type=parsed.ttl", outputType="turtle", deleteEmpty=True, inputFormat=rdfHeadersMapping[self.best_header], logger=self.logger)
    ...
    ontoFiles.parseRDFSource(self.original_file, raw_file_path+"_type=parsed.owl", outputType="rdfxml", deleteEmpty=True, inputFormat=rdfHeadersMapping[self.best_header], logger=self.logger)
...
def handleNewUri(vocab_uri, index, dataPath, source, isNIR, testSuite, logger, user_output=[]):
  ...
  bestHeader, response, triple_number = determine_best_content_type(vocab_uri, user_output=user_output)
